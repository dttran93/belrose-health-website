...
export class VersionControlService {
  ...
  async initializeVersioning(documentId: string, fileObject: FileObject): Promise<string> {
    ...
    const initialVersion: RecordVersion = {
      ...
      fileObjectSnapshot: {
        ...
        recordHash: fileObject.recordHash || null,
        previousRecordHash: fileObject.previousRecordHash || null,
  ...
  async createVersion(
    ...
    if (!currentVersionDoc.exists()) {
      console.error(
        `❌ Current version ${versionControlData.currentVersion} not found for document ${documentId}`
      );
    ...
    console.log(
      'Current version fileObjectSnapshot:',
      JSON.stringify(currentVersion.fileObjectSnapshot, null, 2)
    );
    console.log('Updated version:', JSON.stringify(currentVersion.fileObjectSnapshot, null, 2));
    console.log(
      'New updated file object snapshot for comparison:',
      JSON.stringify(newSnapshot, null, 2)
    );

    // Calculate differences
    const changes = this.deepDiff(currentVersion.fileObjectSnapshot, {
      fhirData: updatedFileObject.fhirData ?? null,
      belroseFields: updatedFileObject.belroseFields ?? null,
      extractedText: updatedFileObject.extractedText ?? null,
      originalText: updatedFileObject.originalText ?? null,
      blockchainVerification: updatedFileObject.blockchainVerification ?? null,
      recordHash: updatedFileObject.recordHash ?? null,
      previousRecordHash: updatedFileObject.previousRecordHash ?? null,
      originalFileHash: updatedFileObject.originalFileHash ?? null,
    });
    ...
    newVersion.changes.forEach((change, index) => {
      ...
      if (change.operation === undefined)
        console.error(`🚨 Found undefined operation in change[${index}]`);
      if (change.path === undefined) console.error(`🚨 Found undefined path in change[${index}]`);
      if (change.fieldType === undefined)
        console.error(`🚨 Found undefined fieldType in change[${index}]`);
      if (change.timestamp === undefined)
        console.error(`🚨 Found undefined timestamp in change[${index}]`);
      if (change.description === undefined)
        console.error(`🚨 Found undefined description in change[${index}]`);
      ...
      if (
        ...
        console.error(
          `🚨 Found undefined newValue in ${change.operation} change[${index}]:`,
          change
        );
      }
    });
    ...
    if (snapshot.belroseFields === undefined)
      console.error(`🚨 Found undefined belroseFields in snapshot`);
    if (snapshot.extractedText === undefined)
      console.error(`🚨 Found undefined extractedText in snapshot`);
    if (snapshot.originalText === undefined)
      console.error(`🚨 Found undefined originalText in snapshot`);
  ...
  async rollbackToVersion(
    ...
    const restoredFileObject: Partial<FileObject> = {
      ...
      recordHash: targetVersion.fileObjectSnapshot.recordHash,
      previousRecordHash: targetVersion.fileObjectSnapshot.previousRecordHash,
      originalFileHash: targetVersion.fileObjectSnapshot.originalFileHash,
    };
    ...
    differences.forEach((change, index) => {
      ...
      if (change.type === 'UPDATE') {
        changes.push({
          ...
          timestamp,
          description: `Updated ${this.pathToHumanReadable(
            change.key || change.path || `change_${index}`
          )}`,
        });
        ...
        changes.push({
          ...
          fieldType: typeof (change.value ?? 'unknown'),
          timestamp,
          description: `Added ${this.pathToHumanReadable(
            change.key || change.path || `change_${index}`
          )}`,
        });
        ...
        changes.push({
          ...
          timestamp,
          description: `Deleted ${this.pathToHumanReadable(
            change.key || change.path || `change_${index}`
          )}`,
        });
      }
  ...
  private generateAutoCommitMessage(changes: ChangeSet[]): string {
    ...
    const parts: string[] = [];
    if (operations.update > 0)
      parts.push(`${operations.update} field${operations.update > 1 ? 's' : ''} updated`);
    if (operations.create > 0)
      parts.push(`${operations.create} field${operations.create > 1 ? 's' : ''} added`);
    if (operations.delete > 0)
      parts.push(`${operations.delete} field${operations.delete > 1 ? 's' : ''} removed`);
  ...
  private async getVersionData(
    documentId: string,
    versionId: string
  ): Promise<{ data: any; timestamp: string } | null> {
  ...
  async getRecentVersions(documentId: string, limitCount = 10): Promise<RecordVersion[]> {
    ...
    const recentQuery = query(versionsRef, orderBy('timestamp', 'desc'), limit(limitCount));
