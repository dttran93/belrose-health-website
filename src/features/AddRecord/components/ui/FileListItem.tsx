...
import type { ValidationMetadata } from '@/features/AddRecord/services/fhirConversionService.type';
import type { FHIRResult } from '../FHIRValidation';

// Remove the duplicate interface definitions since they're now imported from the unified types
...
export const FileListItem: React.FC<FileListItemProps> = ({
  ...
  onRetry,
  onForceConvert,
  onComplete,        // ← Now properly typed as optional
  ...
  const getStatusIcon = (status: FileStatus, fhirResult?: FHIRResult): React.ReactElement | null => {
    switch (status) {
      case 'processing':
  ...
  const getStatusText = (status: FileStatus, fhirResult?: FHIRResult): string => {
  ...
  const handleForceConvert = (): void => {
    if (onForceConvert) {
      onForceConvert(fileItem);
    }
  };
  ...
  // ✅ FIXED: Call onComplete when component mounts and file is completed
  React.useEffect(() => {
