...
function CombinedUploadFHIR({
    ...
    const [fhirValidation, setFhirValidation] = useState(null);
    ...
    // Validate FHIR JSON
    const validateFhirJson = (jsonString) => {
        try {
            ...
            if (parsed.resourceType === 'Bundle') {
                ...
                return { 
                    ...
                    resourceTypes: [...new Set(parsed.entry.map(e => e.resource?.resourceType))]
                };
            } else {
                return { 
                    valid: true, 
    ...
    const handleFhirTextChange = (value) => {
        setFhirText(value);
    ...
    // NEW: Submit FHIR data and immediately upload to Firestore
    const handleFhirSubmit = async () => {
        ...
        try {
            ...
            const uploadResults = await uploadFiles([virtualFile]);
            ...
            alert(`Failed to upload FHIR data: ${error.message}`);
        } finally {
            setSubmitting(false);
        }
    };

    // NEW: Auto-upload files when they finish processing
    const handleFileComplete = async (fileItem) => {
        if (fileItem.status === 'completed' && fileItem.extractedText) {
            try {
                console.log('🚀 Auto-uploading completed file:', fileItem.name);
                const uploadResults = await uploadFiles([fileItem]);
                                ...
                                <div className="space-y-4">
                                    ...
                                    {files.map((fileItem) => (
                                        <FileListItem
                                            ...
                                            onRemove={removeFile}
                                            onRetry={retryFile}
                                            onComplete={handleFileComplete} // NEW: Auto-upload when complete
