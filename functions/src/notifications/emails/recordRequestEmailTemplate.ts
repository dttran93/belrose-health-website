...
function letterhead(badgeText: string, badgeColor: string): string {
  return `
    ...
    <div style="background:#f8fafc;border-radius:0 0 16px 16px;padding:16px 40px;
      ...
      <p style="font-size:11px;color:#94a3b8;margin:0;">
        Questions? <a href="mailto:${SUPPORT_EMAIL}"
          style="color:#64748b;">${SUPPORT_EMAIL}</a>
        &nbsp;·&nbsp;
...
export function buildRecordRequestViewedHtml(targetEmail: string): string {
  return wrap(`
    ...
    <div style="background:#fff;padding:36px 40px;">
      ...
      ${ctaButton(`${APP_URL}/requests`, 'View your requests')}
      <div style="height:1px;background:#f1f5f9;margin:0 0 24px;"></div>
      <p style="font-size:12px;color:#94a3b8;text-align:center;margin:0;">
        Questions? <a href="mailto:${SUPPORT_EMAIL}"
          style="color:#64748b;">${SUPPORT_EMAIL}</a>
        &nbsp;·&nbsp;
        <a href="${MARKETING_URL}" style="color:#64748b;">belrosehealth.com</a>
      </p>
    </div>
    ${footer()}
  `);
...
export function buildRecordRequestViewedText(targetEmail: string): string {
...
export function buildRecordRequestFulfilledHtml(
  targetEmail: string,
  firstRecordId: string | null
): string {
  const recordUrl = firstRecordId ? `${APP_URL}/records/${firstRecordId}` : `${APP_URL}/requests`;
...
export function buildRecordRequestFulfilledText(
  targetEmail: string,
  firstRecordId: string | null
): string {
  const recordUrl = firstRecordId ? `${APP_URL}/records/${firstRecordId}` : `${APP_URL}/requests`;
...
export function buildRecordRequestDeniedHtml(
  targetEmail: string,
...
export function buildRecordRequestDeniedText(
  targetEmail: string,
  deniedReason?: string | null
): string {
