...
interface InitiateHandoffRequest {
  dependentUid: string;
  contactEmail: string;
}

export const initiateHandoff = onCall(
  { secrets: [resendKey] },
  async (request): Promise<{ success: boolean }> => {
    if (!request.auth) throw new HttpsError('unauthenticated', 'Must be authenticated');

    const guardianUid = request.auth.uid;
    const { dependentUid, contactEmail } = request.data as InitiateHandoffRequest;

    if (!dependentUid || !contactEmail) {
      throw new HttpsError('invalid-argument', 'Missing dependentUid or contactEmail');
    }
    ...
    try {
      ...
      throw new HttpsError('not-found', 'Dependent account not found');
    }
    ...
    await db.collection('users').doc(dependentUid).update({
      handoffInitiatedAt: Timestamp.now(),
    });
...
interface HandoffTemplateData {
  displayName: string;
  loginEmail: string;
  guardianName: string;
}

function buildHandoffHtml({ displayName, loginEmail, guardianName }: HandoffTemplateData): string {
...
function buildHandoffText({ displayName, loginEmail, guardianName }: HandoffTemplateData): string {
  const recoveryLink = 'https://belrosehealth.com/auth/recover';
  const loginLink = 'https://belrosehealth.com/auth';
