...
import { resendKey, sendEmailIfEnabled, APP_URL } from '../notifications/emailUtils';

export const claimDependentAccount = onCall(
  { secrets: [resendKey] },
  async (request): Promise<{ success: boolean }> => {
    ...
    if (!userData.isDependent || !userData.dependentCreatedBy) {
      throw new HttpsError('failed-precondition', 'Account is not a dependent account');
    }
    ...
    await createNotification(guardianUid, {
      ...
      link: `${APP_URL}/app/settings/trustees`,
    });
...
function buildClaimEmailHtml(dependentDisplayName: string): string {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="wrapper">
  ...
  <div class="body">
    ...
    <div class="cta-block">
      <a href="${APP_URL}/app/settings/trustees" class="cta-button">View Trustee Settings →</a>
    </div>
  </div>
  <div class="footer">
...
function buildClaimEmailText(dependentDisplayName: string): string {
  return `
...
View your trustee settings: ${APP_URL}/app/settings/trustees
