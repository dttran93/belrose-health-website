...
export function getRefinementAnalyzePrompt(input: RefinementPromptInput): string {
  const {
    fhirData,
    belroseFields,
    extractedText,
...
export function getRefinementRefinePrompt(input: RefinementPromptInput): string {
  const { fhirData, belroseFields, extractedText, originalText, contextText, isSubjectSelf } =
    input;
 ...
 */
function buildReviewSummary(reviewStatus: RecordReviewStatus): string {
  const lines: string[] = [];
  ...
  if (!reviewStatus.hasAnyActiveReview) {
    ...
    if (reviewStatus.hasVerification) {
      ...
      lines.push(
        `- Has active verification ${staleness}, level: ${reviewStatus.verificationLevel}`
      );
    }
