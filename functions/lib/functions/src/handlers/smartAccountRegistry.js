...
exports.registerSmartAccount = void 0;
...
const adminWalletPrivateKey = (0, params_1.defineSecret)('ADMIN_WALLET_PRIVATE_KEY');
 ...
 */
exports.registerSmartAccount = (0, https_1.onCall)({
    ...
    if (!smartAccountAddress) {
        throw new https_1.HttpsError('invalid-argument', 'smartAccountAddress is required');
    }
    if (!ethers_1.ethers.isAddress(smartAccountAddress)) {
        throw new https_1.HttpsError('invalid-argument', 'Invalid smart account address format');
    }
    ...
    const adminWallet = new ethers_1.ethers.Wallet(privateKey, provider);
    const contract = new ethers_1.ethers.Contract(MEMBER_ROLE_MANAGER_ADDRESS, MEMBER_ROLE_MANAGER_ABI, adminWallet);
    ...
    try {
        ...
        await db.collection('users').doc(userId).update({
            ...
            'blockchainMember.smartAccountRegisteredAt': firestore_1.FieldValue.serverTimestamp(),
        });
        return {
            success: true,
            txHash: tx.hash,
        ...
        throw new https_1.HttpsError('internal', `Failed to register smart account: ${error.message}`);
    }
});
//# sourceMappingURL=smartAccountRegistry.js.map